...
###############################################################################
ARRAY_TEST_SCRIPT=$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)
###############################################################################

source "$ARRAY_TEST_SCRIPT/array.sh" ;
...
function test_diff_array_0 {
    local -a LHS=() ;
    local -a RHS=() ;
    assertEquals "" "$(difference LHS RHS)" ;
}

function test_diff_array_1 {
    local -a LHS=(1) ;
    local -a RHS=( ) ;
    assertEquals "1" "$(difference LHS RHS)" ;
    assertEquals "1" "$(difference RHS LHS)" ;
}

function test_diff_array_2 {
    local -a LHS=(1 2) ;
    local -a RHS=(1 2) ;
    assertEquals "" "$(difference LHS RHS)" ;
    assertEquals "" "$(difference RHS LHS)" ;
}

function test_diff_array_21 {
    local -a LHS=(1 2) ;
    local -a RHS=(1  ) ;
    assertEquals "2" "$(difference LHS RHS)" ;
    assertEquals "2" "$(difference RHS LHS)" ;
}

function test_diff_array_22 {
    local -a LHS=(1 2) ;
    local -a RHS=(  2) ;
    assertEquals "1" "$(difference LHS RHS)" ;
    assertEquals "1" "$(difference RHS LHS)" ;
...
function test_diff_array_23 {
    local -a LHS=(1 2) ;
    local -a RHS=(   ) ;
    assertEquals "1 2" "$(difference LHS RHS)" ;
    assertEquals "1 2" "$(difference RHS LHS)" ;
}

function test_diff_array_3 {
    local -a RHS=(0 1 2) ;
    local -a LHS=(0 1 2) ;
    assertEquals "" "$(difference LHS RHS)" ;
    assertEquals "" "$(difference RHS LHS)" ;
...
function test_diff_array_34 {
    local -a LHS=(0 1 2) ;
    local -a RHS=(  1 2) ;
    assertEquals "0" "$(difference LHS RHS)" ;
    assertEquals "0" "$(difference RHS LHS)" ;
}

function test_diff_array_35 {
    local -a LHS=(0 1 2) ;
    local -a RHS=(  1  ) ;
    assertEquals "0 2" "$(difference LHS RHS)" ;
    assertEquals "0 2" "$(difference RHS LHS)" ;
}

function test_diff_array_36 {
    local -a LHS=(0 1 2) ;
    local -a RHS=(    2) ;
    assertEquals "0 1" "$(difference LHS RHS)" ;
    assertEquals "0 1" "$(difference RHS LHS)" ;
}

function test_diff_array_37 {
    local -a LHS=(0 1 2) ;
    local -a RHS=(     ) ;
    assertEquals "0 1 2" "$(difference LHS RHS)" ;
    assertEquals "0 1 2" "$(difference RHS LHS)" ;
}

function test_diff_array_4 {
    local -a LHS=(1 2 3 4 5) ;
    local -a RHS=(5 4 3 2 1) ;
    assertEquals "" "$(difference LHS RHS)" ;
    assertEquals "" "$(difference RHS LHS)" ;
}

function test_diff_array_4a {
    local -a LHS=(1 2 3 4 5) ;
    local -a RHS=(         ) ;
    assertEquals "1 2 3 4 5" "$(difference LHS RHS)" ;
    assertEquals "1 2 3 4 5" "$(difference RHS LHS)" ;
}

function test_diff_array_4b {
    local -a LHS=(5 4 3 2 1) ;
    local -a RHS=(         ) ;
    assertEquals "5 4 3 2 1" "$(difference LHS RHS)" ;
    assertEquals "5 4 3 2 1" "$(difference RHS LHS)" ;
}

function test_diff_array_4c {
    local -a LHS=(1 2   4 5) ;
    local -a RHS=(  4 3 2  ) ;
    assertEquals "1 5 3" "$(difference LHS RHS)" ;
    assertEquals "3 1 5" "$(difference RHS LHS)" ;
}
